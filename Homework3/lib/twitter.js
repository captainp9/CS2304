...
function Tweet(id, content, username, date, retweet, favorites)
{
  this.id = id;
...
function signup(user) {
  if (typeof user === 'string' && user) // valid string && not in the sigh up list
  {
    // High order function 1 use here:
    signupList.forEach(function(element)
    {
      if (element === user)
      {
        return false;
...
function tweet(user, content) {
  ...
  {
    //prototype use here:
    var obj = new Tweet(Math.round((Math.random()+1)*100000),content,user,
              new Date(Date.now()).toISOString(),false,0);

    signupList[user].timeline.unshift(obj);
    //high order function 2 use here:
